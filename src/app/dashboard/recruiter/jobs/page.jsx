...
const MOCK_JOBS = [
  ...
  {
    _id: "job_4",
    jobTitle: "Backend Software Engineer (Node/Go)",
    jobCategory: "technology",
    jobType: "full-time",
    minSalary: "110000",
    maxSalary: "160000",
...
const RecruiterJobs = () => {
  ...
  const companyId = session?.user?.companyId || "company_123";
    ...
    const fetchJobs = async () => {
      try {
        ...
        if (data && Array.isArray(data)) {
          ...
          // If empty array or other weird empty state
          setJobs([]);
        }
      } catch (err) {
        console.warn("Failed to fetch jobs from API, using fallback data:", err.message);
        setError("Could not connect to the job database. Displaying demo listings.");
        setJobs(MOCK_JOBS);
      } finally {
        setLoading(false);
      }
  ...
  const handleDuplicateJob = (job) => {
    ...
    toast.success(`Duplicated "${job.jobTitle}" as a Draft!`);
  };
  ...
  const handleAction = (key, job) => {
    switch (key) {
      case "view_apps":
        ...
        break;
      case "edit":
        toast.error(`Editing details for "${job.jobTitle}" is disabled in demo mode.`);
        break;
      case "toggle_status":
      ...
      case "duplicate":
        ...
        break;
      case "delete":
        if (confirm(`Are you sure you want to delete the job posting for "${job.jobTitle}"?`)) {
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[11px] font-medium text-zinc-500 capitalize">{job.jobCategory}</span>
